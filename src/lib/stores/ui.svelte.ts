...
export interface RetryLastMessageOptions {
...
class UIStore {
  ...
  // One-shot narrative transition guidance, consumed by the next generation
  pendingNarrativeTransitionStoryId = $state<string | null>(null)
  pendingNarrativeTransitionGuidance = $state<string | null>(null)
  ...
  // Chapter text import modal state
  chapterSourceImportModalOpen = $state(false)

  // Novel import modal state
  novelImportModalOpen = $state(false)
  ...
  private retryLastMessageCallback: ((options?: RetryLastMessageOptions) => Promise<void>) | null =
    null
  ...
  queueNarrativeTransitionGuidance(storyId: string, guidance: string) {
    this.pendingNarrativeTransitionStoryId = storyId
    this.pendingNarrativeTransitionGuidance = guidance.trim() || null
  }

  consumeNarrativeTransitionGuidance(storyId: string): string | null {
    if (this.pendingNarrativeTransitionStoryId !== storyId) {
      return null
    }

    const guidance = this.pendingNarrativeTransitionGuidance?.trim() || null
    this.pendingNarrativeTransitionStoryId = null
    this.pendingNarrativeTransitionGuidance = null
    return guidance
  }
  ...
  async triggerRetryLastMessage(options?: RetryLastMessageOptions) {
    console.log('[UI] triggerRetryLastMessage called', {
      hasCallback: !!this.retryLastMessageCallback,
      hasGuidanceNudge: !!options?.guidanceNudge?.trim(),
      hasPreviousNarration: !!options?.previousNarration?.trim(),
    })
    if (this.retryLastMessageCallback) {
      await this.retryLastMessageCallback(options)
  ...
  openChapterSourceImport() {
    this.chapterSourceImportModalOpen = true
  }

  closeChapterSourceImport() {
    this.chapterSourceImportModalOpen = false
  }

  openNovelImport() {
    this.novelImportModalOpen = true
  }

  closeNovelImport() {
    this.novelImportModalOpen = false
  }
