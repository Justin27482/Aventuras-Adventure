import type {
  ...
  ThemeId,
  TranslationSettings,
  ReadingWindowFormattingSettings,
  UISettings,
  UpdateSettings,
} from '$lib/types'
...
export interface ImageGenerationSpecificSettings {
  ...
  smartLoraCatalog: SmartLoraCatalogEntry[]
  maxAutoLoras: number
}
...
export function getDefaultExperimentalFeatures(): ExperimentalFeatures {
  return {
    ...
    epistemicCostOverlayEnabled: false,
  }
}
...
const READING_WINDOW_FORMATTING_DEFAULTS: ReadingWindowFormattingSettings = {
  mainText: '#e2e8f0',
  italicsText: '#93c5fd',
  doubleAsteriskText: '#f59e0b',
  quoteText: '#f472b6',
  angleBracketText: '#34d399',
}
...
function clampVaultLorebookTextBaseRem(value: number): number {
  return Math.min(
    ...
    Math.max(VAULT_LOREBOOK_TEXT_BASE_REM_MIN, roundToTenth(value)),
  )
}

function getDefaultReadingWindowFormattingSettings(): ReadingWindowFormattingSettings {
  return { ...READING_WINDOW_FORMATTING_DEFAULTS }
}
...
export function getDefaultUISettings(): UISettings {
  return {
    ...
    storyMaxWidth: '3xl',
    vaultLorebookEditorWidth: 'compact',
    vaultLorebookEditorTextBaseRem: VAULT_LOREBOOK_TEXT_BASE_REM_DEFAULT,
    readingWindowFormatting: getDefaultReadingWindowFormattingSettings(),
  }
}
...
export const DEFAULT_SERVICE_PRESET_ASSIGNMENTS: Record<string, string> = {
  ...
  stImportProcessing: 'classification',
  editorAssistant: 'memory',
  memory: 'memory',
  chapterQuery: 'memory',
  timelineFill: 'memory',
  suggestions: 'suggestions',
  actionChoices: 'suggestions',
  disclosureGate: 'classification',
  directorOutliningAssistant: 'agentic',
...
class SettingsStore {
  ...
  apiSettings = $state<APISettings>({
    ...
    openRouterContextCompressionEnabled: false,
  ...
  async init() {
    ...
    try {
      ...
      const openRouterContextCompressionEnabled = await database.getSetting(
        'openrouter_context_compression_enabled',
      )
      if (openRouterContextCompressionEnabled !== null) {
        this.apiSettings.openRouterContextCompressionEnabled =
          openRouterContextCompressionEnabled === 'true'
      }
        ...
        try {
          ...
          if (this.uiSettings.fontSource === 'google' && this.uiSettings.fontFamily !== 'default') {
            try {
              ...
              console.warn('[Settings] Failed to load Google Font during init:', error)
            }
          }
      ...
      const vaultLorebookEditorWidth = await database.getSetting('vault_lorebook_editor_width')
      if (
        vaultLorebookEditorWidth &&
        VALID_VAULT_LOREBOOK_EDITOR_WIDTHS.includes(
          vaultLorebookEditorWidth as UISettings['vaultLorebookEditorWidth'],
        )
      ) {
        this.uiSettings.vaultLorebookEditorWidth =
          vaultLorebookEditorWidth as UISettings['vaultLorebookEditorWidth']
      }

      const vaultLorebookEditorTextBaseRem = await database.getSetting(
        'vault_lorebook_editor_text_base_rem',
      )
      if (vaultLorebookEditorTextBaseRem) {
        const parsed = Number(vaultLorebookEditorTextBaseRem)
        if (Number.isFinite(parsed)) {
          this.uiSettings.vaultLorebookEditorTextBaseRem = clampVaultLorebookTextBaseRem(parsed)
        }
      } else {
        // Migration from old enum setting
        const legacyTextSize = await database.getSetting('vault_lorebook_editor_text_size')
        if (legacyTextSize === 'large') {
          this.uiSettings.vaultLorebookEditorTextBaseRem = 1.1
        } else if (legacyTextSize === 'normal') {
          this.uiSettings.vaultLorebookEditorTextBaseRem = 1.0
        }
      }

      const readingWindowMainText = await database.getSetting('reading_window_main_text')
      if (readingWindowMainText !== null) {
        this.uiSettings.readingWindowFormatting.mainText = readingWindowMainText
      }

      const readingWindowItalicsText = await database.getSetting('reading_window_italics_text')
      if (readingWindowItalicsText !== null) {
        this.uiSettings.readingWindowFormatting.italicsText = readingWindowItalicsText
      }

      const readingWindowDoubleAsteriskText = await database.getSetting(
        'reading_window_double_asterisk_text',
      )
      if (readingWindowDoubleAsteriskText !== null) {
        this.uiSettings.readingWindowFormatting.doubleAsteriskText = readingWindowDoubleAsteriskText
      }

      const readingWindowQuoteText = await database.getSetting('reading_window_quote_text')
      if (readingWindowQuoteText !== null) {
        this.uiSettings.readingWindowFormatting.quoteText = readingWindowQuoteText
      }

      const readingWindowAngleBracketText = await database.getSetting(
        'reading_window_angle_bracket_text',
      )
      if (readingWindowAngleBracketText !== null) {
        this.uiSettings.readingWindowFormatting.angleBracketText = readingWindowAngleBracketText
      }
  ...
  async setVaultLorebookEditorWidth(width: UISettings['vaultLorebookEditorWidth']) {
    this.uiSettings.vaultLorebookEditorWidth = width
    await database.setSetting('vault_lorebook_editor_width', width)
  }

  async setVaultLorebookEditorTextBaseRem(value: number) {
    const clamped = clampVaultLorebookTextBaseRem(value)
    this.uiSettings.vaultLorebookEditorTextBaseRem = clamped
  ...
  async setReadingWindowFormatting(updates: Partial<ReadingWindowFormattingSettings>) {
    ...
    const keyMap: Partial<Record<keyof ReadingWindowFormattingSettings, string>> = {
      mainText: 'reading_window_main_text',
      italicsText: 'reading_window_italics_text',
      doubleAsteriskText: 'reading_window_double_asterisk_text',
      quoteText: 'reading_window_quote_text',
      angleBracketText: 'reading_window_angle_bracket_text',
    }

    await Promise.all(
      Object.entries(updates).map(([key, value]) =>
        database.setSetting(keyMap[key as keyof ReadingWindowFormattingSettings]!, value),
      ),
    )
  }
  ...
  async updateExperimentalFeatures(updates: Partial<ExperimentalFeatures>) {
    ...
    // Keep execution mode within known values if a malformed value was injected.
    if (
      updates.epistemicExecutionMode &&
      updates.epistemicExecutionMode !== 'quality' &&
      updates.epistemicExecutionMode !== 'fast'
    ) {
      updates.epistemicExecutionMode = 'quality'
    }

    const effectiveWorkflowEnabled =
      updates.epistemicWorkflowEnabled ?? this.experimentalFeatures.epistemicWorkflowEnabled
    const effectiveExecutionMode =
      updates.epistemicExecutionMode ?? this.experimentalFeatures.epistemicExecutionMode

    // Contract: disclosure gate always runs in fast mode when workflow is enabled.
    if (effectiveWorkflowEnabled && effectiveExecutionMode === 'fast') {
      updates.epistemicDisclosureGateEnabled = true
    }
  ...
  async resetAllSettings(preserveApiSettings = true) {
    ...
    await database.setSetting(
      'vault_lorebook_editor_width',
      this.uiSettings.vaultLorebookEditorWidth,
    )
    await database.setSetting(
      'vault_lorebook_editor_text_base_rem',
      this.uiSettings.vaultLorebookEditorTextBaseRem.toFixed(1),
    )
    await database.setSetting(
      'reading_window_main_text',
      this.uiSettings.readingWindowFormatting.mainText,
    )
    await database.setSetting(
      'reading_window_italics_text',
      this.uiSettings.readingWindowFormatting.italicsText,
    )
    await database.setSetting(
      'reading_window_double_asterisk_text',
      this.uiSettings.readingWindowFormatting.doubleAsteriskText,
    )
    await database.setSetting(
      'reading_window_quote_text',
      this.uiSettings.readingWindowFormatting.quoteText,
    )
    await database.setSetting(
      'reading_window_angle_bracket_text',
      this.uiSettings.readingWindowFormatting.angleBracketText,
    )
    await database.setSetting(
