...
import { stImportAIProcessingService } from '$lib/services/stImport/aiProcessingService'
...
export class STImportWizardStore {
  ...
  detectedPOV = $state<POV | null>(null)
  detectedTense = $state<Tense | null>(null)
  styleDetectionConfidence = $state<'low' | 'medium' | 'high' | null>(null)
  styleDetectionRationale = $state<string | null>(null)
  isDetectingStyle = $state(false)
  styleDetectionError = $state<string | null>(null)
  hasManualStyleOverride = $state(false)
  enableStyleRewritePass = $state(false)
  enableCreativeCleanupPass = $state(false)
  ...
  importProcessingStatus = $state<string | null>(null)
  importProcessingProgress = $state<{ phase: string; current: number; total: number } | null>(null)
  ...
  async processChatFile(text: string) {
    this.chatFileError = null
    this.styleDetectionError = null
    ...
    // Detect dominant POV/tense from imported chat and preselect when untouched.
    if (result.messages.length > 0) {
      this.isDetectingStyle = true
      try {
        const detected = await stImportAIProcessingService.detectStyle(
          result.messages,
          this.selectedPackId,
        )
        this.detectedPOV = detected.pov
        this.detectedTense = detected.tense
        this.styleDetectionConfidence = detected.confidence
        this.styleDetectionRationale = detected.rationale || null

        if (!this.hasManualStyleOverride) {
          this.selectedPOV = detected.pov
          this.selectedTense = detected.tense
        }
      } catch (err) {
        this.styleDetectionError = describeAIError(err)
      } finally {
        this.isDetectingStyle = false
      }
    }
  }
  ...
  clearChatFile() {
    ...
    this.detectedPOV = null
    this.detectedTense = null
    this.styleDetectionConfidence = null
    this.styleDetectionRationale = null
    this.styleDetectionError = null
    this.isDetectingStyle = false
    this.enableStyleRewritePass = false
    this.enableCreativeCleanupPass = false
    this.importProcessingStatus = null
    this.importProcessingProgress = null
  }

  setSelectedPOV(pov: POV) {
    this.selectedPOV = pov
    this.hasManualStyleOverride = true
  }

  setSelectedTense(tense: Tense) {
    this.selectedTense = tense
    this.hasManualStyleOverride = true
  }

  setEnableStyleRewritePass(enabled: boolean) {
    this.enableStyleRewritePass = enabled
  }

  setEnableCreativeCleanupPass(enabled: boolean) {
    this.enableCreativeCleanupPass = enabled
  }
  ...
  async processCardImport() {
    ...
    try {
      ...
      if (this.importCharacters) {
        ...
        // Default protagonist to the imported card character so ST imports
        // naturally create stories about that character.
        this.protagonist = {
          name: cardChar.name,
          description: cardChar.description,
          background: '',
          motivation: '',
          traits: cardChar.traits,
        }
        this.protagonistPortrait = this.cardPortrait
        ...
        this.supportingCharacters = dedupedNpcs
      }
  ...
  setSupportingCharacterAsProtagonist(index: number) {
    const selected = this.supportingCharacters[index]
    if (!selected) return

    const nextSupporting = this.supportingCharacters.filter((_, i) => i !== index)
    const previousProtagonist = this.protagonist
    const previousProtagonistPortrait = this.protagonistPortrait
    ...
    this.protagonistPortrait = this.characterPortraits.get(selected.name) ?? null
    this.showManualInput = false

    if (
      previousProtagonist &&
      previousProtagonist.name.trim().toLowerCase() !== selected.name.trim().toLowerCase()
    ) {
      const previousAsSupporting: GeneratedCharacter = {
        name: previousProtagonist.name,
        role: 'supporting',
        description: previousProtagonist.description || '',
        relationship: '',
        traits: previousProtagonist.traits || [],
      }

      if (
        !nextSupporting.some(
          (c) => c.name.trim().toLowerCase() === previousAsSupporting.name.trim().toLowerCase(),
        )
      ) {
        nextSupporting.unshift(previousAsSupporting)
      }

      if (previousProtagonistPortrait && !this.characterPortraits.has(previousAsSupporting.name)) {
        const map = new SvelteMap(this.characterPortraits)
        map.set(previousAsSupporting.name, previousProtagonistPortrait)
        this.characterPortraits = map
      }
    }

    this.supportingCharacters = nextSupporting
  }
  ...
  async createStory() {
    ...
    try {
      ...
      if (this.importChatAsEntries && this.chatParseResult) {
        ...
        // Import the full transcript so the opening AI message remains visible in chat.
        const messagesToImport = this.chatParseResult.messages

        if (messagesToImport.length > 0) {
          const processedMessages = await this.applyImportProcessingPasses(messagesToImport)
          await story.importSTChat(processedMessages)
  ...
  private async applyImportProcessingPasses(messages: STChatMessage[]): Promise<STChatMessage[]> {
    ...
    const runPass = async (
      phaseLabel: string,
      transform: (chunk: STChatMessage[]) => Promise<STChatMessage[]>,
    ) => {
      const total = Math.ceil(current.length / chunkSize)
      const next: STChatMessage[] = []

      for (let i = 0; i < total; i++) {
        this.importProcessingStatus = `${phaseLabel} (${i + 1}/${total})`
        this.importProcessingProgress = { phase: phaseLabel, current: i + 1, total }
    ...
    try {
      if (this.enableStyleRewritePass) {
        await runPass('Rewriting imported style', (chunk) =>
          stImportAIProcessingService.rewriteBatch(
            chunk,
            this.selectedPackId,
            this.selectedPOV,
            this.selectedTense,
      ...
      if (this.enableCreativeCleanupPass) {
        await runPass('Cleaning imported text', (chunk) =>
          stImportAIProcessingService.cleanupBatch(chunk, this.selectedPackId),
        )
      }
    } catch (err) {
    ...
    this.importProcessingStatus =
      this.enableStyleRewritePass || this.enableCreativeCleanupPass
        ? 'Import text processing complete.'
        : null
    return current
  }
