...
### Money tracking - ONLY when money tracking is active:
- If the narration clearly changes the protagonist's money, set scene.moneyUpdate.
- If the passage contains a transaction cue (buy/sell/pay/spend/cost/reward/loot/wage/fee/bribe) and an explicit amount or currency mention, you MUST set scene.moneyUpdate.
- Use moneyUpdate.delta as the net change:
  - Negative for spending/paying/fees/bribes/losses
  - Positive for rewards/loot/wages/sales/found money
- Use moneyUpdate.transactionType with the closest category.
- If the narration shows an attempted purchase that fails for lack of money, set:
  - deniedPurchase = true
  - attemptedCost when known
  - delta = 0
- If no clear money event happened, omit moneyUpdate.
{% endif %}

{% if inventoryClassificationEnabled and clothingSystemInstructions != '' %}
### Clothing damage and repair - ONLY when clothing state is being tracked:
- If an already-known clothing item is torn, shredded, ruined, knocked off, repaired, or made to expose/cover specific body zones, update that EXISTING item via itemUpdates.changes.clothingState.
- Use damageSeverity for the intensity of the damage event:
  - minor = light tearing/scuffing/strain
  - moderate = obvious tearing or damage
  - major = severe shredding or heavy destruction
  - destroyed = effectively ruined, removed, or no longer providing coverage
- Use newlyExposedZones for zones that become exposed in this passage.
- Use restoredZones for zones that become covered again due to repair, re-fastening, or adjustment.
- Use repaired=true when the narration clearly repairs or restores the clothing item.
- Do NOT create a new item just because existing clothing was damaged; update the existing clothing item instead.
{% endif %}

{% if inventoryClassificationEnabled %}
**Acquired container rule (IMPORTANT):**
- If a character receives/opens a container (satchel, chest, pouch, backpack, etc.) and the narration lists contents, extract each listed content item as acquired unless the text clearly says they did not take it.
- In this case, default new item location to "inventory" unless worn/equipped is explicitly stated.
- Example: "He opens the satchel: rope, lockpicks, herbs, dagger" = extract all four as new items.
{% endif %}
...
{% endif %}
{% if locationClassificationEnabled or sceneClassificationEnabled %}
...
{% if clothingSystemInstructions != '' %}
{{ clothingSystemInstructions }}
{% endif %}

{% if moneySystemInstructions != '' %}
{{ moneySystemInstructions }}
{% endif %}
...
## Your Task
{% if characterClassificationEnabled or locationClassificationEnabled or inventoryClassificationEnabled or storyBeatClassificationEnabled %}
1. Check if any EXISTING enabled entities need updates (status change, new info learned, etc.)
2. Identify any NEW significant entities introduced for enabled subprocesses (apply extraction rules strictly)
{% endif %}
{% if storyBeatClassificationEnabled %}
3. **IMPORTANT**: Check if any active story beats have been COMPLETED or FAILED in this passage - mark them accordingly
{% endif %}
{% if sceneClassificationEnabled %}
4. Determine the current scene state
{% endif %}
{% if timeClassificationEnabled %}
5. Determine time progression for this passage
{% endif %}
{% if moneyClassificationEnabled %}
6. If money cues are present, populate scene.moneyUpdate
{% endif %}
...
const classifierMoneyRecoveryPromptTemplate: PromptTemplate = {
...
export const analysisTemplates: PromptTemplate[] = [
  ...
  classifierMoneyRecoveryPromptTemplate,
]
