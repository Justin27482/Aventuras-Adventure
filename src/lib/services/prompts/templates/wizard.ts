...
const stImportStyleDetectionPromptTemplate: PromptTemplate = {
  id: 'st-import-style-detection',
  name: 'ST Import Style Detection',
  category: 'wizard',
  description: 'Detects dominant POV and tense in imported ST chat content',
  content: `You are a narrative style analyzer.

Task:
- Infer the dominant narrative point-of-view (POV) and tense from imported story messages.
- Pick exactly one POV: first, second, or third.
- Pick exactly one tense: past or present.

Guidelines:
- Use dominant usage across the sample, not one-off lines.
- Ignore OOC/meta fragments, headers, and malformed lines.
- If mixed, choose the most probable dominant style and reduce confidence.
- Keep rationale concise and concrete.

Return structured JSON only via the schema.`,
  userContent: `Analyze this imported ST chat sample and detect its dominant style.

MESSAGES SAMPLE:
{{ messagesSample }}`,
}

const stImportStyleRewritePromptTemplate: PromptTemplate = {
  id: 'st-import-style-rewrite',
  name: 'ST Import Style Rewrite',
  category: 'wizard',
  description: 'Rewrites imported ST chat messages into selected POV and tense',
  content: `You are a narrative line editor for imported chat logs.

Task:
- Rewrite each input message into the target narrative style.
- Preserve factual content, sequence, named entities, and intent.
...
Hard rules:
- Keep output count equal to input count.
- Keep each output mapped to its original index.
- Preserve message type intent: user_action lines stay action-like; narration lines stay narrative.
- Remove obvious OOC wrappers only when needed for fluent prose.
- Avoid adding author notes or explanations.

Return structured JSON only via the schema.`,
  userContent: `Rewrite this batch of imported messages.

INPUT BATCH:
{{ messagesJson }}`,
}

const stImportCreativeCleanupPromptTemplate: PromptTemplate = {
  id: 'st-import-creative-cleanup',
  name: 'ST Import Creative Cleanup',
  category: 'wizard',
  description: 'Cleans imported ST chat by removing AI/meta artifacts while preserving story canon',
  content: `You are cleaning imported roleplay logs for creative writing continuity.

Task:
- Remove or rewrite obvious AI-chat artifacts and meta chatter.
- Preserve narrative canon, facts, chronology, and character intent.
- Keep immersive in-world prose.

Examples of content to remove or rewrite when present:
- OOC/meta notes, model disclaimers, instruction leakage, format directives.
- Conversation-management lines that are not part of story canon.

Rules:
- Keep output count equal to input count.
- Keep each output mapped to its original index.
- If a line is already clean, keep it close to original.
- Do not add new plot facts.

Return structured JSON only via the schema.`,
  userContent: `Clean this batch of imported messages for creative-writing continuity.
...
export const wizardTemplates: PromptTemplate[] = [
  ...
  stImportStyleRewritePromptTemplate,
  stImportCreativeCleanupPromptTemplate,
]
