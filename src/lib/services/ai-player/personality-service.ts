...
export function deriveEffectiveAIPlayerTraits(
  ...
  const immersionBase = Number.isFinite(base.immersion) ? base.immersion : 5
  const arousalBase = Number.isFinite(base.arousal) ? base.arousal : 0

  let immersion = immersionBase
  let arousal = arousalBase

  if (/atmosphere|detailed|sensory|terrain|weather|location|environment|investigate|explore|observe|focus|setting|ambience|texture|mood/.test(text)) {
    immersion += 1
  }
  if (/romance|flirt|kiss|touch|seductive|intimate|tease|desire|lust|passion|heat|embrace|bed|sultry/.test(text)) {
    arousal += 1
  }
  if (/cold|dismissive|boring|flat|awkward|distance|disinterest|fear|banter|joke|sarcasm/.test(text)) {
...
function relationshipForPlayer(
  relationship: AIPlayerRelationship,
  aiPlayerId: string,
): string {
  const otherPlayerId =
