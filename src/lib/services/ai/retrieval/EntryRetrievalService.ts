...
export interface EntryRetrievalOptions {
  storyMode?: StoryMode
  audience?: EntryRetrievalAudience
}
...
export class EntryRetrievalService extends BaseAIService {
  ...
  async getRelevantEntries(
    ...
    const storyMode = options.storyMode ?? 'adventure'
    const audience = options.audience ?? 'player_safe'
    ...
    const candidateEntries = visibleEntries.filter(
    ...
    // Build context block
    const contextBlock = this.buildContextBlock(tier1, tier2, tier3, audience)
...
(CANONICAL - All information below is established lore. Do not contradict these facts.)`
    ...
    if (byType.character.length > 0) {
      ...
      for (const { entry } of byType.character) {
        const description = this.truncateEntryText(this.getAudienceDescription(entry, audience))
  ...
  private isEntryVisibleForAudience(
    entry: Entry,
    storyMode: StoryMode,
    audience: EntryRetrievalAudience,
  ): boolean {
    const visibilityScope = entry.visibilityScope ?? 'both'
    const secrecyScope = entry.secrecyScope ?? 'public'
    ...
    if (secrecyScope === 'director_only') {
      return false
    }

    if (secrecyScope !== 'public' && revealState !== 'revealed') {
      return false
    }
