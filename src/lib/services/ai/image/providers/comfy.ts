...
import BasicImg2ImgWorkflow from './comfyWorkflows/basic-img2img-workflow.json'
...
export const ComfyModes: Record<ComfyMode, any> = {
  ...
  [ComfyMode.LoraTxt2Img]: BasicTxt2ImgWorkflow,
...
type ComfyLoraOption = {
  name: string
  strengthModel?: number
  strengthClip?: number
}

function normalizeLoraOptions(providerOptions?: Record<string, unknown>): ComfyLoraOption[] {
  const normalized: ComfyLoraOption[] = []

  const rawList = providerOptions?.loras
  if (Array.isArray(rawList)) {
    for (const item of rawList) {
      if (!item || typeof item !== 'object') continue
      const candidate = item as Partial<ComfyLoraOption>
  ...
  const legacy = providerOptions?.lora
  if (legacy && typeof legacy === 'object') {
    const candidate = legacy as Partial<ComfyLoraOption>
...
export function createComfyProvider(config: ImageProviderConfig): ImageProvider {
  ...
  const extractChoiceList = (value: unknown): string[] => {
  ...
  return {
    ...
    async generate(options: ImageGenerateOptions): Promise<ImageGenerateResult> {
      const { prompt, size, providerOptions, referenceImages } = options
      ...
      const loraOptions = normalizeLoraOptions(providerOptions)

      // Explicit mode always wins. Auto-detection only runs when no mode is set.
      const hasExplicitOverride = !!explicitMode
      const isLoraMode =
        explicitMode === ComfyMode.LoraTxt2Img ||
        (explicitMode === ComfyMode.BasicTxt2Img && loraOptions.length > 0) ||
        (!hasExplicitOverride && loraOptions.length > 0)
      ...
      if (isUnetMode) {
        ...
        const denoise =
          typeof providerOptions?.denoise === 'number' ? providerOptions.denoise : 0.65
        const hasReferenceImage = Array.isArray(referenceImages) && referenceImages.length > 0
        ...
        const workflowBase = JSON.parse(
          JSON.stringify(hasReferenceImage ? BasicImg2ImgWorkflow : BasicTxt2ImgWorkflow),
        ) as any

        const loraNodeIds: string[] = []
        if (isLoraMode && loraOptions.length > 0) {
          const existingNodeIds = Object.keys(workflowBase).map((id) => Number(id))
          let nextNodeId = Math.max(...existingNodeIds, 0) + 1
          let previousModelNodeId = '4'
          let previousClipNodeId = '4'

          for (const lora of loraOptions) {
            const nodeId = String(nextNodeId++)
            workflowBase[nodeId] = {
              class_type: 'LoraLoader',
              _meta: { title: `load_lora_${loraNodeIds.length + 1}` },
              inputs: {
                lora_name: lora.name,
                strength_model: lora.strengthModel ?? 1,
                strength_clip: lora.strengthClip ?? 1,
                model: [previousModelNodeId, 0],
                clip: [previousClipNodeId, 1],
              },
            }
            loraNodeIds.push(nodeId)
            previousModelNodeId = nodeId
            previousClipNodeId = nodeId
          }

          const samplerInputs = workflowBase['3']?.inputs
          const positiveClipInputs = workflowBase['6']?.inputs
          const negativeClipInputs = workflowBase['7']?.inputs

          if (!samplerInputs || !positiveClipInputs || !negativeClipInputs) {
            throw new Error('ComfyUI workflow missing expected sampler/CLIP nodes for LoRA mode.')
          }

          samplerInputs.model = [previousModelNodeId, 0]
          positiveClipInputs.clip = [previousClipNodeId, 1]
          negativeClipInputs.clip = [previousClipNodeId, 1]
        }

        for (let i = 0; i < loraNodeIds.length; i++) {
          inputKeys.push(`lora_name_${i}`, `lora_strength_model_${i}`, `lora_strength_clip_${i}`)
        }
        ...
        if (hasReferenceImage) {
          ...
          const imageInfo = uploaded.info as { filename: string; subfolder?: string }
          const imagePath = imageInfo.subfolder
            ? `${imageInfo.subfolder}/${imageInfo.filename}`
            : imageInfo.filename

          builder = builder
            .setInputNode('image', '11.inputs.image')
            .setInputNode('denoise', '3.inputs.denoise')
            .input('image', imagePath, api.osType)
        ...
        if (isLoraMode && loraOptions.length > 0) {
          for (let i = 0; i < loraNodeIds.length; i++) {
            const lora = loraOptions[i]
            const nodeId = loraNodeIds[i]
            builder = builder
              .setInputNode(`lora_name_${i}`, `${nodeId}.inputs.lora_name`)
              .setInputNode(`lora_strength_model_${i}`, `${nodeId}.inputs.strength_model`)
              .setInputNode(`lora_strength_clip_${i}`, `${nodeId}.inputs.strength_clip`)
              .input(`lora_name_${i}`, lora.name, api.osType)
              .input(`lora_strength_model_${i}`, lora.strengthModel ?? 1)
              .input(`lora_strength_clip_${i}`, lora.strengthClip ?? 1)
          }
        }
    ...
    async listModels(): Promise<ImageModelInfo[]> {
      const [checkpoints, diffusionModels, textEncoders, vaeModels] = await Promise.all([
        ...
        fetchModels('diffusion_models').catch(() => [] as string[]),
        fetchModels('text_encoders').catch(() => [] as string[]),
        fetchModels('vae').catch(() => [] as string[]),
      ])
      ...
      const toInfo =
        (prefix: string, description: string, supportsImg2Img: boolean) =>
      ...
      return [
        ...checkpoints.map(toInfo('checkpoint', 'Checkpoint', true)),
        ...diffusionModels.map(toInfo('unet', 'Diffusion Model', false)),
      ]
    },
    ...
    async listEmbeddings(): Promise<string[]> {
      try {
        return await fetchModels('embeddings')
      } catch {
        return []
      }
    },
    ...
    async getSamplerInfo(): Promise<ComfySamplerInfo> {
      let raw: any = null
      try {
        raw = await api.getSamplerInfo()
      } catch {
        // Some ComfyUI builds do not expose the legacy sampler endpoint the SDK uses.
      }

      const legacySamplers = extractChoiceList(raw?.sampler)
      const legacySchedulers = extractChoiceList(raw?.scheduler)
      if (legacySamplers.length > 0 || legacySchedulers.length > 0) {
        return {
          samplers: legacySamplers,
          schedulers: legacySchedulers,
        }
      }
      ...
      try {
        ...
        return {
          samplers: extractChoiceList(required?.sampler_name),
          schedulers: extractChoiceList(required?.scheduler),
        }
      } catch {
