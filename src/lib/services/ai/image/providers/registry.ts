...
export async function generateImage(options: {
  ...
  providerOptionsOverride?: Record<string, unknown>
  signal?: AbortSignal
...
/**
 * List available embeddings for a ComfyUI provider.
 */
export async function listEmbeddings(baseUrl?: string): Promise<string[]> {
  try {
    const config: ImageProviderConfig = {
      apiKey: '',
      baseUrl,
    ...
    const provider = createComfyProvider(config)
    if (provider.listEmbeddings) {
      return await provider.listEmbeddings()
    }
    return []
  } catch (error) {
    log('Error getting embeddings list', { error })
    return []
  }
}
