...
export interface WorldState {
  ...
  clothingSystemEnabled?: boolean
  moneySystemEnabled?: boolean
  moneyName?: string
  currentMoney?: number
...
function zoneExposurePriority(exposedZones: string[]): number {
  if (exposedZones.some((zone) => SENSITIVE_ZONES.has(zone))) return 1
  if (exposedZones.some((zone) => MODERATE_ZONES.has(zone))) return 2
  if (exposedZones.some((zone) => MINOR_ZONES.has(zone))) return 3
  return 3
}

function clothingSortPriority(metadata?: Record<string, any>): number {
  const exposedZones = normalizeZones(metadata?.exposedZones)
  const coveredZones = normalizeZones(metadata?.coveredZones)
  const effectiveCoveredZones = coveredZones.filter((zone) => !exposedZones.includes(zone))
  const durability = typeof metadata?.durability === 'number' ? metadata.durability : null
  const maxDurability = typeof metadata?.maxDurability === 'number' ? metadata.maxDurability : null
  const unusable = Boolean(metadata?.unusable)

  if (unusable || (effectiveCoveredZones.length === 0 && exposedZones.length > 0)) return 0
  if (exposedZones.length > 0) return zoneExposurePriority(exposedZones)
  if (durability !== null && maxDurability !== null && durability < maxDurability) return 4
  return 5
}

function formatClothingState(metadata?: Record<string, any>): string {
  const exposedZones = normalizeZones(metadata?.exposedZones)
  const coveredZones = normalizeZones(metadata?.coveredZones)
  const effectiveCoveredZones = coveredZones.filter((zone) => !exposedZones.includes(zone))
  const durability = typeof metadata?.durability === 'number' ? metadata.durability : null
  ...
  const parts: string[] = []
  if (unusable || (effectiveCoveredZones.length === 0 && exposedZones.length > 0)) {
    parts.push('removed/destroyed')
  } else if (exposedZones.length > 0) {
    parts.push(`exposed ${exposedZones.join(', ')}`)
  } else if (durability !== null && maxDurability !== null && durability < maxDurability) {
    parts.push('damaged but still covering')
  } else {
    parts.push('intact')
  }

  if (effectiveCoveredZones.length > 0 && exposedZones.length > 0) {
...
export class EntryInjector extends BaseAIService {
    ...
    for (const item of inventoryItems.slice(0, this.config.maxEntriesPerTier)) {
      ...
      entries.push({
        ...
        metadata: {
          ...
          isClothing: clothing?.isClothing ?? false,
          coveredZones: clothing?.coveredZones ?? [],
          exposedZones: clothing?.exposedZones ?? [],
          durability: clothing?.durability ?? null,
          maxDurability: clothing?.maxDurability ?? null,
          unusable: clothing?.unusable ?? false,
        },
      })
    }
  ...
  private buildContextBlock(
    ...
    const equippedClothingItems = inventoryItems
      .filter((item) => item.metadata?.equipped && item.metadata?.isClothing)
      .sort((a, b) => {
    ...
    if (worldState.clothingSystemEnabled && equippedClothingItems.length > 0) {
      block += '\n\n[CLOTHING STATE]'
      for (const item of equippedClothingItems) {
        const status = formatClothingState(item.metadata)
        block += `\n• ${item.name}${status ? ` - ${status}` : ''}`
      }
    }

    if (worldState.moneySystemEnabled) {
      const moneyName = (worldState.moneyName ?? 'gold').trim() || 'gold'
      const currentMoney = Math.max(0, Math.floor(worldState.currentMoney ?? 0))
