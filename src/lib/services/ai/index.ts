...
import { DisclosureGateService, EntryInjector, MemoryService, NarrativeService } from './generation'
import type {
...
class AIService {
  ...
  private disclosureGateService: DisclosureGateService

  constructor() {
    this.narrativeService = serviceFactory.createNarrativeService()
    this.disclosureGateService = serviceFactory.createDisclosureGateService()
  }
  ...
  async generateNarrative(
    ...
    const draft = await this.narrativeService.generate(entries, worldState, story)
    return this.applyDisclosureGateIfNeeded(draft, story, worldState)
  }

  /**
   * Apply a manual editor pass to an existing narrative response.
   */
  async applyManualEditorPass(
    ...
    return this.narrativeService.applyManualEditorPass(draft, worldState, story, { signal })
  }
  ...
  async *streamNarrative(
    ...
    const shouldGateBeforeDisplay =
      !!currentStory &&
      settings.experimentalFeatures.epistemicWorkflowEnabled &&
      settings.experimentalFeatures.epistemicDisclosureGateEnabled

    if (!shouldGateBeforeDisplay) {
    ...
    for await (const chunk of this.narrativeService.stream(entries, worldState, currentStory, {
      tieredContextBlock,
      styleReview,
      retrievedChapterContext,
      signal,
      ...
      if (chunk.reasoning) bufferedReasoning += chunk.reasoning
      if (chunk.done) break
    }

    const gatedContent = await this.applyDisclosureGateIfNeeded(
      bufferedContent,
      currentStory,
      worldState,
    )
  ...
  private async applyDisclosureGateIfNeeded(
    narrative: string,
    story: Story | null | undefined,
    worldState: WorldState,
  ): Promise<string> {
    if (!story) return narrative
    if (!settings.experimentalFeatures.epistemicWorkflowEnabled) return narrative
    if (!settings.experimentalFeatures.epistemicDisclosureGateEnabled) return narrative
    if (!narrative.trim()) return narrative

    const secretAtoms = await database.getSecretAtomsForStory(story.id)
    const protectedAtoms = secretAtoms.filter(
      (atom) => atom.revealState !== 'revealed' || atom.secrecyScope !== 'public',
    )

    const hasLegacyHiddenEntries = (worldState.lorebookEntries ?? []).some(
      (entry) => !!entry.hiddenInfo?.trim(),
    )

    if (protectedAtoms.length === 0 && !hasLegacyHiddenEntries) {
      return narrative
    }

    const knowledgeEdges = await database.getKnowledgeEdgesForStory(story.id)
    const result = await this.disclosureGateService.gateNarrative({
      storyMode: story.mode,
      narrative,
      characters: worldState.characters,
      lorebookEntries: worldState.lorebookEntries ?? [],
      secretAtoms: protectedAtoms,
      knowledgeEdges,
    })

    if (result.action === 'suppress') {
      return result.revisedContent.trim()
    }

    return result.revisedContent?.trim() ? result.revisedContent : narrative
  }
  ...
  /**
   * Estimate the in-story time delta for a chapter summary.
   * This is a deterministic fallback used by batch chapterization.
   */
  ...
  async getRelevantLorebookEntries(
    ...
    const result = await entryService.getRelevantEntries(
      ...
      activationTracker,
      signal,
      { storyMode, audience: 'player_safe' },
    )
