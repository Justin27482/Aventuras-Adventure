...
const clothingStateChangeSchema = z.object({
  damageSeverity: z
    .enum(['minor', 'moderate', 'major', 'destroyed'])
    .describe('Severity of clothing damage when narration tears, shreds, ruins, or removes it')
    .optional(),
  newlyExposedZones: z
    .array(z.string())
    .describe('Body zones newly exposed by this narrative event')
    .optional(),
  restoredZones: z
    .array(z.string())
    .describe('Previously exposed zones that are covered again due to repair or re-adjustment')
    .optional(),
  repaired: z
    .boolean()
    .describe('true if the clothing was explicitly repaired or restored in this passage')
    .optional(),
})
...
export const itemUpdateSchema = z.object({
  ...
  changes: z.object({
    ...
    clothingState: clothingStateChangeSchema
      .describe('Use only for clothing damage, exposure, destruction, or repair state changes')
      .optional(),
  }),
})
...
const moneyUpdateSchema = z.object({
  delta: z
    .number()
  ...
  deniedPurchase: z
    .boolean()
    .describe('true if an attempted purchase was denied due to insufficient funds')
    .optional(),
  attemptedCost: z.number().describe('Cost attempted for a denied purchase').optional(),
})
...
export const sceneSchema = z.object({
  ...
  timeProgression: z
    ...
    .default('none'),
  moneyUpdate: moneyUpdateSchema
    .describe('Only include when a clear money gain/loss/transaction happened this passage')
    .optional(),
})
...
export interface ClassificationSchemaOptions {
  characterClassificationEnabled?: boolean
  locationClassificationEnabled?: boolean
  inventoryClassificationEnabled?: boolean
  storyBeatClassificationEnabled?: boolean
  sceneClassificationEnabled?: boolean
  timeClassificationEnabled?: boolean
  moneyClassificationEnabled?: boolean
}
...
export function buildClassificationResultSchema(
  ...
  if (!options.characterClassificationEnabled) {
    entry = entry.omit({ characterUpdates: true, newCharacters: true })
  }
  if (!options.locationClassificationEnabled) {
    entry = entry.omit({ locationUpdates: true, newLocations: true })
  }
  if (!options.inventoryClassificationEnabled) {
    entry = entry.omit({ itemUpdates: true, newItems: true })
  }
  if (!options.storyBeatClassificationEnabled) {
    entry = entry.omit({ storyBeatUpdates: true, newStoryBeats: true })
  }

  if (!options.sceneClassificationEnabled) {
    scene = scene.omit({ currentLocationName: true, presentCharacterNames: true })
  }
  if (!options.timeClassificationEnabled) {
    scene = scene.omit({ timeProgression: true })
  }
  if (!options.moneyClassificationEnabled) {
    scene = scene.omit({ moneyUpdate: true })
  }

  return z.object({ entryUpdates: entry, scene })
}
...
export type ClassificationResult = z.infer<typeof classificationResultSchema> & {
  ...
  /** Internal metadata: classifier LLM extraction failed and fallbacks were used. Not LLM output. */
  _classifierFallbackUsed?: boolean
  /** Internal metadata: best-effort error message when classifier extraction failed. Not LLM output. */
  _classifierError?: string
  /** Internal metadata: narrative had strong money cues but classifier omitted scene.moneyUpdate. */
  _moneyExtractionMissed?: boolean
  /** Internal metadata: short reason for money extraction miss diagnostics. */
  _moneyExtractionMissReason?: string
}
