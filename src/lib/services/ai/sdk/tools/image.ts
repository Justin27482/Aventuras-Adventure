...
import { selectSmartLoras } from '$lib/services/ai/image/smartLora'
...
export function createImageTools(context: ImageToolContext) {
  ...
  function resolveSmartLoras(params: {
    profileId: string | null
    prompt: string
    characterId?: string
  }): Array<{ name: string; strengthModel: number; strengthClip: number; reason: string }> {
    const { profileId, prompt, characterId } = params
    if (!settings.experimentalFeatures.smartLoraRouting) return []
    if (!profileId) return []

    const profile = settings.getImageProfile(profileId)
    if (!profile || profile.providerType !== 'comfyui') return []

    const character = characterId ? characters().find((c) => c.id === characterId) : undefined
    const maxAutoLoras = settings.serviceSpecificSettings.imageGeneration.maxAutoLoras ?? 3

    return selectSmartLoras({
      catalog: settings.serviceSpecificSettings.imageGeneration.smartLoraCatalog,
      profileId,
      prompt,
      character,
      maxAutoLoras,
    ...
    generate_standard_image: tool({
      ...
      inputSchema: z.object({
        ...
        characterId: z
          .string()
          .optional()
          .describe('Optional character ID for context-aware smart LoRA routing'),
      }),
      execute: async ({ prompt, characterId }: { prompt: string; characterId?: string }) => {
        ...
        try {
          ...
          const selectedLoras = resolveSmartLoras({ profileId, prompt: fullPrompt, characterId })
          ...
          if (result.base64) {
            ...
            const loraSummary =
              selectedLoras.length > 0
                ? ` Applied LoRAs: ${selectedLoras.map((l) => l.name).join(', ')}.`
                : ''
            return {
              success: true,
              imageId,
              message: `Image generated (imageId: ${imageId}). The image is displayed in the chat. Use imageId with set_portrait to assign it as a character portrait.${loraSummary}`,
            }
          }
      ...
      execute: async ({ characterId }: { characterId: string }) => {
        ...
        try {
          ...
          if (descriptors.trim()) {
            ...
            const selectedLoras = resolveSmartLoras({
            ...
            const base64 = await sdkGeneratePortrait(portraitPrompt, providerOptionsOverride)
            ...
            return {
              ...
              imageId,
              characterName: character.name,
              message: `Portrait for "${character.name}" completed (imageId: ${imageId}). The image is displayed in the chat. Use imageId with set_portrait to assign it as the character's portrait.${loraSummary}`,
            }
          }
        } catch (error) {
