  ...
  if (!settings.advancedRequestSettings.manualMode) {
    switch (providerType) {
      case 'openrouter':
        ...
        if (settings.apiSettings.openRouterContextCompressionEnabled) {
          options = {
            ...options,
...
export async function generateStructured<T extends z.ZodType>(
  ...
  const withReasoningLimitProviderOptions: ProviderOptions | undefined = (() => {
    if (!reasoningEffortOverride && !reasoningMaxTokensOverride) {
    ...
    if (providerType === 'openrouter') {
      const openrouter = (next.openrouter ?? {}) as Record<string, unknown>
      const reasoning = (openrouter.reasoning ?? {}) as Record<string, unknown>

      // OpenRouter requires exactly one reasoning mode:
      // either `effort` OR `max_tokens`.
      if (typeof reasoningMaxTokensOverride === 'number' && reasoningMaxTokensOverride > 0) {
        delete reasoning.effort
        delete reasoning.reasoning_effort
        reasoning.max_tokens = Math.floor(reasoningMaxTokensOverride)
      } else if (reasoningEffortOverride && reasoningEffortOverride !== 'off') {
        delete reasoning.max_tokens
        reasoning.effort = reasoningEffortOverride
      } else if (reasoningEffortOverride === 'off') {
        delete reasoning.max_tokens
        delete reasoning.effort
    ...
    return next as ProviderOptions
  })()

  const runStructured = async (
    callProviderOptions: ProviderOptions | undefined,
    ...
    const result = await generateText({
      model: wrapLanguageModel({
        ...
        middleware: buildStructuredMiddleware(
          supportsStructuredOutput,
          useThinkTag,
          !reasoningOff && !!preset.reasoningEffort && preset.reasoningEffort !== 'off',
  ...
  const mergedReasoningOffProviderOptions: ProviderOptions | undefined = {
    ...(providerOptions ?? {}),
    openrouter: {
      ...((providerOptions as Record<string, unknown> | undefined)?.openrouter as
  ...
  const shouldRetryWithoutReasoning = (error: unknown): boolean => {
    if (NoObjectGeneratedError.isInstance(error)) return true
    if (error instanceof Error) {
      return /no\s+output\s+generated/i.test(error.message)
    }
    return false
  }

  try {
    return await runStructured(withReasoningLimitProviderOptions, !!forceReasoningOff)
  } catch (error) {
    if (forceReasoningOff || !shouldRetryWithoutReasoning(error)) {
      throw error
    }
    log('Structured output failed; retrying once with reasoning disabled', {
      presetId,
      model: preset.model,
      providerType,
      error: error instanceof Error ? error.message : String(error),
    })
    return await runStructured(mergedReasoningOffProviderOptions, true)
  }
}
