...
class ExportService {
  ...
  private logVersionCompatibilityWarnings(importVersion: string): void {
    ...
    if (this.compareVersions(importVersion, '1.9.0') < 0) {
      console.warn(
        `[Import] File from v${importVersion} predates chapter source records (v1.9.0). Raw chapter text sources will not be restored.`,
      )
    }
  }
  ...
  async importFromContent(
    ...
    try {
      ...
      // Import chapter source records (added in v1.9.0)
      if (data.chapterSources) {
        for (const source of data.chapterSources) {
          await database.addChapterSource({
            id: crypto.randomUUID(),
            storyId: newStoryId,
            branchId: mapBranchId(source.branchId ?? null),
            title: source.title,
            ...
            plotThreads: source.plotThreads ?? [],
            emotionalTone: source.emotionalTone ?? null,
            sourceType: source.sourceType ?? 'import',
