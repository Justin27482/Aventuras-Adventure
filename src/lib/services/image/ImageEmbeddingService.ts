...
import {
  encodeReadingWindowFormatting,
  restoreReadingWindowFormatting,
...
function processUnified(
  ...
  applyReadingWindowFormatting = true,
): string {
  if (images.length === 0 && !content.includes('<pic')) {
    if (!applyReadingWindowFormatting) {
      return render(content)
    }

    const formattedText = encodeReadingWindowFormatting(content)
    const html = render(formattedText.text)
    return restoreReadingWindowFormatting(html, formattedText.replacements)
  }
  ...
  let html: string
  if (applyReadingWindowFormatting) {
    const formattedText = encodeReadingWindowFormatting(text)
    html = render(formattedText.text)
    html = restoreReadingWindowFormatting(html, formattedText.replacements)
  } else {
    html = render(text)
  }
...
export function processStoryContent(
  ...
  return processUnified(content, images, regeneratingIds, parseMarkdown, true)
}
