 ...
 * 3. Restores current location, time tracker, and money from the earliest rolled-back delta
...
class RollbackService {
  ...
  async rollbackFromPosition(
    ...
    // Track the earliest delta's before-state for location/time/money restoration
    ...
    let earliestMoneyAmount: number | null | undefined = undefined
    ...
    for (const entry of entriesToRollback) {
      ...
      earliestMoneyAmount = delta.previousState.moneyAmount
    }
    ...
    // 5. Restore money amount from the earliest rolled-back delta
    if (earliestMoneyAmount !== undefined) {
      try {
        const story = await database.getStory(storyId)
        if (story) {
          const nextSettings = {
            ...(story.settings ?? {}),
            moneyAmount: earliestMoneyAmount ?? 0,
          }
          await database.updateStory(storyId, { settings: nextSettings })
          log('Money amount restored to', earliestMoneyAmount)
        }
      } catch (error) {
        console.error('[RollbackService] Failed to restore money amount:', error)
      }
    }

    // 6. Clean up auto-snapshots after the rollback position
    try {
    ...
    // 7. Clean up no-op COW overrides (overrides whose data matches the original)
    try {
