    ...
    <ScrollArea class="min-h-0 flex-1">
      <div class="space-y-6 p-6">
        {#if ui.lastLorebookRetrieval}
          ...
          <!-- Scope Note -->
          <Card class="border-blue-500/30 bg-blue-500/5 shadow-sm">
            <CardContent class="space-y-3 p-4">
              <p class="text-sm font-medium text-blue-700 dark:text-blue-300">
                This panel shows lorebook context only (active entries and injected context block).
              </p>
              <p class="text-muted-foreground text-xs">
                It does not include raw LLM request/response payloads. Use API Debug Logs for that.
              </p>
              <div class="flex flex-wrap items-center gap-2">
                <Button size="sm" variant="secondary" onclick={() => debug.openDebugModal()}>
                  Open API Debug Logs
                </Button>
                {#if !settings.uiSettings.debugMode}
                  <span class="text-xs text-amber-600 dark:text-amber-400">
                    Enable Debug Mode in Settings > Advanced to capture new requests.
                  </span>
                {/if}
              </div>
            </CardContent>
