<script lang="ts">
  ...
  } from '$lib/services/migrations/migration-catalog'
  ...
  async function checkMissingMigrations() {
    ...
    const appliedVersions = new Set(
      statuses.filter((status) => status.success).map((status) => status.version),
    )
    missingVersions = MIGRATION_CATALOG.filter(
  ...
  async function installNextMigration() {
    ...
    try {
      ...
      await database.installMigration({
        ...
        previousVersions: MIGRATION_CATALOG.filter((migration) => migration.version < version).map(
...
<ResponsiveModal.Root open={ui.migrationLogOpen} onOpenChange={(open) => !open && backToSettings()}>
    ...
    <ResponsiveModal.Header class="border-border border-b px-6 py-4">
      <div class="flex items-center justify-between gap-4">
        ...
        <Button
          variant="outline"
          size="sm"
          onclick={checkMissingMigrations}
          disabled={loading}
          title="Compare migration files with the applied ledger"
        >
    ...
    <div class="flex-1 overflow-auto p-4 sm:p-6">
      ...
      {:else}
        {#if missingVersions !== null}
          <div
            class={`mb-4 rounded-md border p-3 text-sm ${missingVersions.length > 0 ? 'border-amber-500/30 bg-amber-500/10 text-amber-200' : 'border-emerald-500/30 bg-emerald-500/10 text-emerald-200'}`}
          >
            {#if missingVersions.length > 0}
              <p class="font-medium">
                {missingVersions.length} migration{missingVersions.length === 1 ? '' : 's'} missing from
                the applied ledger.
              </p>
              <p class="mt-1 text-xs">Missing versions: {missingVersions.join(', ')}</p>
              <p class="mt-1 text-xs">
                Only the next missing migration can be installed, one at a time. New migrations use
                retry-safe incremental execution so a lock retry can resume without repeating
                destructive work.
              </p>
              <p class="mt-1 text-xs">
                An attempt can fail when an earlier migration is missing, the migration is already
                recorded, a required table or column is absent, the SQL is incompatible with the
                current database, or the checksum cannot be recorded. The error below will identify
                the reason.
              </p>
              <Button
                class="mt-3"
                onclick={installNextMigration}
                disabled={installingVersion !== null}
              >
                <RefreshCw
                  class={installingVersion !== null ? 'h-4 w-4 animate-spin' : 'h-4 w-4'}
                />
                {installingVersion !== null
                  ? `Installing ${installingVersion}...`
                  : `Attempt Install Migration ${missingVersions[0]}`}
              </Button>
              {#if installMessage}
                <div
                  class="mt-3 rounded border border-amber-500/30 bg-black/10 p-2 text-xs whitespace-pre-wrap"
                >
                  {installMessage}
                </div>
              {/if}
              <div class="mt-3 space-y-1">
                {#each missingVersions as version (version)}
                  {@const catalog = MIGRATION_CATALOG.find(
                    (migration) => migration.version === version,
                  )}
                  {#if catalog}
                    <div
                      class="flex items-center gap-2 rounded border border-amber-500/20 px-2 py-1.5"
                    >
                      <button
                        class="text-muted-foreground"
                        onclick={() => toggle(version)}
                        title="Expand missing migration details"
                      >
                        {#if expanded.has(version)}<ChevronDown
                            class="h-4 w-4"
                      ...
                      <Button variant="outline" size="sm" onclick={() => openSql(catalog)}
                        ><Code2 class="h-3.5 w-3.5" /> View SQL</Button
                      >
                    </div>
                    {#if expanded.has(version)}
                      <div
                        class="space-y-2 rounded border border-amber-500/20 bg-black/10 px-8 py-2 text-xs"
                      >
                        <div><span class="text-muted-foreground">Status:</span> Not applied</div>
                        <div>
                          <span class="text-muted-foreground">Affected objects:</span>
                          {catalog.affectedObjects.join(', ') || 'None detected'}
                        </div>
                      </div>
                    {/if}
                  {/if}
            ...
            <div class="border-border border-b last:border-b-0">
              <div class="hover:bg-muted/40 flex items-center gap-3 px-3 py-2.5">
                <button
                  ...
                  onclick={() => toggle(row.status.version)}
                  title="Expand migration details"
                >
                  {#if expanded.has(row.status.version)}<ChevronDown
                      class="h-4 w-4"
                    />{:else}<ChevronRight class="h-4 w-4" />{/if}
                </button>
                {#if row.status.success}<CheckCircle2
                    class="h-4 w-4 shrink-0 text-emerald-500"
                  />{:else}<CircleAlert class="text-destructive h-4 w-4 shrink-0" />{/if}
                <span class="w-12 shrink-0 font-mono text-xs">{row.status.version}</span>
                <span class="min-w-0 flex-1 truncate text-sm font-medium"
                  >{row.status.description || row.catalog?.description || 'Unknown migration'}</span
                >
                <Badge variant={row.status.success ? 'secondary' : 'destructive'}
                  >{row.status.success ? 'Applied' : 'Failed'}</Badge
                >
                <span class="text-muted-foreground hidden text-xs sm:inline"
                  >{row.status.executionTimeMs ?? '?'} ms</span
                >
              </div>
                ...
                <div class="bg-muted/20 space-y-3 border-t px-10 py-3 text-xs">
                  <div class="grid gap-2 sm:grid-cols-2">
                    <div>
                      ...
                      {row.status.installedOn ?? 'Unknown'}
                    </div>
                    <div>
                      <span class="text-muted-foreground">Checksum:</span>
                      <span class="font-mono">{row.status.checksum ?? 'Unavailable'}</span>
                    </div>
                  </div>
                  <div>
                    <span class="text-muted-foreground">Affected objects:</span>
                    {row.catalog?.affectedObjects.join(', ') || 'Catalog unavailable'}
                  </div>
                  {#if row.catalog}
  ...
  <Dialog.Content class="flex max-h-[85vh] flex-col sm:max-w-4xl">
    <Dialog.Header>
      <Dialog.Title class="flex items-center gap-2"
        ><Code2 class="h-4 w-4" /> Migration {selectedSql?.version} SQL</Dialog.Title
      >
      <Dialog.Description>{selectedSql?.filename}</Dialog.Description>
    </Dialog.Header>
    <pre
      class="bg-muted max-h-[60vh] overflow-auto rounded-md p-4 text-left font-mono text-xs leading-relaxed whitespace-pre-wrap">{selectedSql?.sql}</pre>
    <Dialog.Footer
      ><Button variant="outline" onclick={() => (showSql = false)}>Close</Button></Dialog.Footer
    >
  </Dialog.Content>
</Dialog.Root>
