...
<script lang="ts">
  ...
  async function handleSmartLoraRoutingToggle(checked: boolean) {
    await settings.updateExperimentalFeatures({ smartLoraRouting: checked })
  }
    ...
    <div class="flex flex-row items-center justify-between">
      <div class="space-y-0.5">
        <div class="flex items-center gap-2">
          ...
          <Label>Smart LoRA Routing</Label>
        </div>
        <p class="text-muted-foreground text-xs">
          Allow Vault Assistant image generation to automatically apply LoRAs from your catalog
          using character and prompt context (including age rules).
        </p>
        {#if settings.experimentalFeatures.smartLoraRouting}
          <p class="pt-1 text-xs font-medium text-amber-500">
            Active — catalog rules may auto-apply one or more LoRAs.
          </p>
        {/if}
      </div>
      <Switch
        checked={settings.experimentalFeatures.smartLoraRouting}
        onCheckedChange={handleSmartLoraRoutingToggle}
      />
    </div>
  </div>
