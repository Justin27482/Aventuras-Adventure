<script lang="ts">
  ...
  const isMainProfileOpenRouter = $derived(
    settings.getMainNarrativeProfile()?.providerType === 'openrouter',
  )

  async function handleOpenRouterContextCompressionToggle(checked: boolean) {
    settings.apiSettings.openRouterContextCompressionEnabled = checked
  ...
  <Card.Content class="grid gap-3 pt-4">
    ...
    {#if isMainProfileOpenRouter}
      <div class="mt-2 flex flex-row items-center justify-between rounded-lg border border-surface-700 bg-surface-900/40 px-3 py-2">
        ...
        <Switch
          checked={settings.apiSettings.openRouterContextCompressionEnabled}
          onCheckedChange={handleOpenRouterContextCompressionToggle}
        />
      </div>
    {/if}
