<script lang="ts">
  ...
  import {
    ...
    renderReadingWindowContent,
  } from '$lib/utils/readingWindowFormatting'
  import {
    processStoryContent,
  ...
  const guidedRegenDisabledReason = $derived.by(() => {
  ...
  let guidedRegenOpen = $state(false)
  let guidedRegenInput = $state('')
  ...
  const guidedRegenSentenceCount = $derived(countSentences(guidedRegenInput))
  const canSubmitGuidedRegen = $derived(
    canRetry && guidedRegenSentenceCount >= 1 && guidedRegenSentenceCount <= 3,
  )

  function openGuidedRegen() {
    guidedRegenInput = ''
    guidedRegenOpen = true
  }

  async function submitGuidedRegen() {
    if (!canSubmitGuidedRegen || ui.isGenerating) return
    const guidance = guidedRegenInput.trim()
    guidedRegenOpen = false
    await ui.triggerRetryLastMessage({
      guidanceNudge: guidance,
      previousNarration: entry.content,
    })
  }
  ...
  const isEditDirty = $derived(editContent.trim() !== entry.content)
  ...
  function handleContentClick(event: MouseEvent | KeyboardEvent) {
    ...
    // Editing is explicit via the toolbar Edit button.
    // Avoid opening the editor from content clicks because it conflicts with image actions.
  }
  ...
  async function placeEditorCursor(position: number) {
    ...
    const clamped = Math.max(0, Math.min(position, editContent.length))
    editTextarea.focus()
    editTextarea.setSelectionRange(clamped, clamped)

    const lineHeightRaw = Number.parseFloat(getComputedStyle(editTextarea).lineHeight)
    const lineHeight = Number.isFinite(lineHeightRaw) ? lineHeightRaw : 20
    const linesBefore = editContent.slice(0, clamped).split('\n').length - 1
    editTextarea.scrollTop = Math.max(0, (linesBefore - 2) * lineHeight)
  }

  function startEdit(cursorPosition?: number) {
    editContent = entry.content
    isEditing = true
    if (cursorPosition !== undefined) {
      void placeEditorCursor(cursorPosition)
    }
  }
  ...
  async function handleManualEditorPass() {
    ...
    isApplyingEditorPass = true
    try {
      const draft = (entry.translatedContent ?? entry.content).trim()
      if (!draft) {
        ui.showToast('Cannot run editor pass on empty narration.', 'error')
        return
      }

      const edited = await aiService.applyManualEditorPass(
        draft,
        {
      ...
      if (!normalized) {
        ui.showToast('Editor pass returned empty content. Entry was unchanged.', 'error')
        return
      }

      if (normalized === entry.content) {
        ui.showToast('Editor pass made no changes.', 'info')
        return
      }

      await story.updateEntry(entry.id, normalized)
      ui.showToast('Editor pass applied.', 'success')
    } catch (error) {
      console.error('[StoryEntry] Manual editor pass failed:', error)
      ui.showToast(error instanceof Error ? error.message : 'Editor pass failed.', 'error')
    } finally {
      isApplyingEditorPass = false
    }
  }
  ...
  $effect(() => {
    ...
    const onPointerDown = (event: PointerEvent) => {
      ...
      if (!target || editContainer.contains(target)) return

      // Only auto-abort when there are no unsaved changes.
      if (!isEditDirty) {
        cancelEdit()
      }
    }
...
>
      ...
      <div class="flex items-center gap-0.5">
        ...
        {#if isLatestNarration}
          <Button
            variant="text"
            size="icon"
            onclick={openGuidedRegen}
            disabled={!canRetry}
            class="h-7 w-7 text-amber-500 hover:text-amber-600"
            title={canRetry ? 'Guided regenerate' : guidedRegenDisabledReason}
          >
            <MessageSquarePlus class="h-4 w-4" />
          </Button>
        {/if}
        {#if canManualEditorPass}
          <Button
            variant="text"
            size="icon"
            onclick={handleManualEditorPass}
            disabled={ui.isGenerating || isApplyingEditorPass}
            class="h-7 w-7 text-sky-500 hover:text-sky-600"
            title={isApplyingEditorPass
              ? 'Applying editor pass...'
              : ui.isGenerating
                ? 'Wait for current generation to finish'
                : 'Polish this response (manual editor pass)'}
          >
            {#if isApplyingEditorPass}
  ...
  <div class="min-w-0">
    {#if isEditing}
      <div class="space-y-2" bind:this={editContainer}>
        <div class="relative">
          <Textarea
            bind:ref={editTextarea}
            bind:value={editContent}
            onkeydown={handleKeydown}
            class="min-h-25 w-full resize-y pb-12 text-base"
            rows={4}
          />
          {#if isEditDirty}
            <Button
              size="sm"
              onclick={saveEdit}
              class="absolute right-3 bottom-3 h-8 px-3 shadow-md"
            >
    ...
    {:else}
      ...
      {#if canManualEditorPass && !isErrorEntry}
        <div class="mt-3 flex items-center gap-2">
          <Button
            ...
            onclick={handleManualEditorPass}
            disabled={ui.isGenerating || isApplyingEditorPass}
            class="h-8 px-3 text-sky-600 hover:text-sky-700"
            title={isApplyingEditorPass
              ? 'Applying editor pass...'
              : ui.isGenerating
                ? 'Wait for current generation to finish'
                : 'Trigger a manual editor pass for this finished response'}
          >
            {#if isApplyingEditorPass}
              <Loader2 class="mr-1.5 h-3.5 w-3.5 animate-spin" />
              Applying Editor Pass...
            {:else}
              <WandSparkles class="mr-1.5 h-3.5 w-3.5" />
              Editor Pass
            {/if}
          </Button>
        </div>
...
<ResponsiveModal.Root bind:open={guidedRegenOpen}>
  <ResponsiveModal.Content class="sm:max-w-lg">
    <ResponsiveModal.Header
      title="Guided Regenerate"
      description="Add 1-3 sentences to gently steer the regenerated reply."
    />

    <div class="space-y-3 px-1 pb-1">
      <Textarea
        bind:value={guidedRegenInput}
        rows={4}
        maxlength={500}
        placeholder="Example: Keep it grounded to this town's economy. Mention realistic jobs and plausible pay, and avoid introducing formal contracts."
      />
      <div class="flex items-center justify-between text-xs">
        <span class="text-muted-foreground">{guidedRegenSentenceCount} / 3 sentences</span>
        {#if guidedRegenSentenceCount === 0 || guidedRegenSentenceCount > 3}
          <span class="text-amber-500">Enter 1-3 sentences.</span>
        {/if}
      </div>
    </div>

    <ResponsiveModal.Footer>
      <Button variant="secondary" onclick={() => (guidedRegenOpen = false)}>Cancel</Button>
      <Button onclick={submitGuidedRegen} disabled={!canSubmitGuidedRegen || ui.isGenerating}
        >Regenerate</Button
      >
    </ResponsiveModal.Footer>
