<script lang="ts">
  ...
  let draft = $derived(campaign.settings?.worldCharter ?? '')
...
<Card>
  ...
  <CardContent class="space-y-3">
    <p class="text-muted-foreground text-xs">
      Campaign truths, boundaries, faction tensions, open mysteries, and director guidance.
    </p>
    <div class="flex flex-wrap gap-2">
      <Button
        variant="outline"
        size="sm"
        onclick={draftFromCampaign}
        disabled={!input || saving || expanding}
      >
        {#if saving}<Loader2 class="h-3.5 w-3.5 animate-spin" />{:else}<Sparkles
            class="h-3.5 w-3.5"
          />{/if}
        Draft from Campaign
      </Button>
      <Button
        ...
        disabled={!input || saving || expanding}
      >
        {#if expanding}<Loader2 class="h-3.5 w-3.5 animate-spin" />{:else}<Sparkles
            class="h-3.5 w-3.5"
          />{/if}
        Expand with AI
      </Button>
    ...
    <Textarea
      bind:value={draft}
      rows={12}
      placeholder="Define the campaign premise, immutable facts, factions, open questions, and boundaries the GM should preserve."
    />
    <div class="flex items-center justify-between gap-3">
      {#if error}<p class="text-destructive text-xs">{error}</p>{:else if saved}<p
      ...
      <Button
        size="sm"
        onclick={() => void save()}
        disabled={!campaign.settings || saving || expanding}>Save Charter</Button
      >
    </div>
  </CardContent>
</Card>
