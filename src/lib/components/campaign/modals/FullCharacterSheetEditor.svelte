<script lang="ts">
  ...
  let editing = $state<CharacterSheetDraft>({
    name: '',
    description: '',
    traits: [],
    visualDescriptors: {},
  ...
  let appearanceText = $state('{}')
  let error = $state<string | null>(null)

  $effect(() => {
    if (!open) return
    const nextDraft = copyCharacterSheetDraft(draft)
    const nextRulesetId = ruleset?.ruleset?.id ?? nextDraft.sheet.rulesetId
    editing = { ...nextDraft, sheet: { ...nextDraft.sheet, rulesetId: nextRulesetId } }
    ...
    try {
      ...
      error =
        reason instanceof Error
          ? reason.message
          : String(reason || 'Unable to save character sheet')
    }
  }
</script>
...
<Dialog.Root {open} onOpenChange={(value) => !value && onClose()}>
  <Dialog.Content class="flex max-h-[92vh] flex-col sm:max-w-5xl">
    <Dialog.Header>
      ...
      <Dialog.Description
        >Review and edit the complete character definition and mechanical sheet.</Dialog.Description
      >
    </Dialog.Header>
    <div class="grid min-h-0 flex-1 gap-4 overflow-y-auto lg:grid-cols-[1.4fr_0.6fr]">
      <div class="space-y-4">
        <section class="grid gap-3 rounded-md border p-3 sm:grid-cols-2">
          <label class="space-y-1 text-xs font-medium"
            >Name<Input bind:value={editing.name} /></label
          >
          <label class="space-y-1 text-xs font-medium"
            >Traits<Input bind:value={traitsText} placeholder="Comma separated" /></label
          >
          <label class="space-y-1 text-xs font-medium sm:col-span-2"
            >Description<Textarea bind:value={editing.description} rows={4} /></label
          >
          <label class="space-y-1 text-xs font-medium sm:col-span-2"
            >Appearance values (JSON)<Textarea
              bind:value={appearanceText}
              rows={7}
            ...
            {#each ruleset.stats as stat (stat.key)}
              <label class="space-y-1 text-xs font-medium"
                ...
                <Input
                  ...
                  min={stat.minValue}
                  max={stat.maxValue}
                  bind:value={editing.sheet.statValues[stat.key]}
                />
              </label>
            {/each}
          </div>
        ...
        <section class="space-y-3 rounded-md border p-3">
          <div class="grid gap-3 sm:grid-cols-2">
            <label class="space-y-1 text-xs font-medium"
              >Level<Input type="number" min="1" bind:value={editing.sheet.level} /></label
            >
            <label class="space-y-1 text-xs font-medium"
              >XP<Input type="number" min="0" bind:value={editing.sheet.xp} /></label
            >
          </div>
            ...
            {#each ruleset.resources as resource (resource.key)}
              <div class="grid grid-cols-2 gap-2 rounded-md border p-2">
                <label class="space-y-1 text-xs"
                  >{resource.label} current<Input
                    type="number"
                    min="0"
                    bind:value={editing.sheet.resourceValues[resource.key].current}
                  /></label
                >
                <label class="space-y-1 text-xs"
                  >Maximum<Input
                    type="number"
                    min="0"
                    bind:value={editing.sheet.resourceValues[resource.key].max}
                  /></label
                >
              </div>
            {/each}
          ...
          <div class="space-y-2">
            {#each ruleset.conditions as condition (condition.key)}
              <label class="flex items-center gap-2 rounded-md border p-2 text-xs">
                <input
                  type="checkbox"
                  bind:checked={editing.sheet.conditionStates[condition.key].active}
                />
                <span class="min-w-28 font-medium">{condition.label}</span>
                <Input
                  bind:value={editing.sheet.conditionStates[condition.key].note}
      ...
      <aside class="space-y-2 rounded-md border p-3">
        <h3 class="flex items-center gap-2 text-sm font-semibold">
          <History class="h-4 w-4" /> Revision History
        </h3>
          ...
          {#each [...revisions].reverse() as revision (revision.id)}
            <div class="space-y-1 rounded-md border p-2 text-xs">
              <div class="font-medium">
                {revision.authorType === 'gm' ? 'GM edit' : 'Approved AI proposal'}
              </div>
    ...
    <Dialog.Footer>
      ...
      <Button onclick={() => void save()} disabled={busy}
        ><Save class="h-4 w-4" /> {busy ? 'Saving...' : saveLabel}</Button
      >
    </Dialog.Footer>
  </Dialog.Content>
</Dialog.Root>
