<script lang="ts">
  ...
  const entryEditorWidth = $derived(settings.uiSettings.vaultLorebookEditorWidth)
  const entryEditorTextBaseRem = $derived(settings.uiSettings.vaultLorebookEditorTextBaseRem)
  const entryEditorTextPseudoPt = $derived(Math.round(entryEditorTextBaseRem * 10))
  const entryEditorWidthClass = $derived.by(() => {
    if (entryEditorWidth === 'wide') return 'max-w-5xl'
    if (entryEditorWidth === 'comfortable') return 'max-w-4xl'
    return 'max-w-3xl'
  })

  async function nudgeEntryEditorTextBaseRem(deltaRem: number) {
    const next = Math.round((entryEditorTextBaseRem + deltaRem) * 10) / 10
    await settings.setVaultLorebookEditorTextBaseRem(next)
  }

  async function setEntryEditorPseudoPt(value: number) {
    if (!Number.isFinite(value)) return
    const rounded = Math.round(value)
    const clamped = Math.min(18, Math.max(6, rounded))
    await settings.setVaultLorebookEditorTextBaseRem(clamped / 10)
  }
  ...
  /** Discard all local unsaved edits (entries + settings) without closing the editor. */
          ...
          >
            {#if selectedEntry !== null && selectedIndex !== null}
                  ...
                  <div class="hidden items-center gap-2 md:flex">
                    ...
                    >
                      ...
                      <button
                        class={cn(
                          ...
                          entryEditorWidth === 'compact'
                            ? 'bg-surface-600 text-surface-100'
                            : 'text-surface-400 hover:text-surface-200',
                        )}
                        onclick={() => void settings.setVaultLorebookEditorWidth('compact')}
                      >
                        S
                      </button>
                      <button
                        class={cn(
                          'rounded px-1.5 py-0.5 text-[10px] transition-colors',
                          entryEditorWidth === 'comfortable'
                            ? 'bg-surface-600 text-surface-100'
                            : 'text-surface-400 hover:text-surface-200',
                        )}
                        onclick={() => void settings.setVaultLorebookEditorWidth('comfortable')}
                      >
                        M
                      </button>
                      <button
                        class={cn(
                          'rounded px-1.5 py-0.5 text-[10px] transition-colors',
                          entryEditorWidth === 'wide'
                            ? 'bg-surface-600 text-surface-100'
                            : 'text-surface-400 hover:text-surface-200',
                        )}
                        onclick={() => void settings.setVaultLorebookEditorWidth('wide')}
                      >
                        L
                      </button>
                    </div>
                    ...
                    >
                      ...
                      <button
                        ...
                        onclick={() => void nudgeEntryEditorTextBaseRem(-0.1)}
                        disabled={entryEditorTextBaseRem <= 0.6}
                        title="Decrease text size"
                      >
                        -
                      </button>
                      <span
                        class="text-surface-300 inline-flex min-w-[3.25rem] items-center justify-center px-1 text-[10px] font-semibold"
                        title={`${entryEditorTextBaseRem.toFixed(1)}rem`}
                      >
                        <input
                          type="number"
                          min="6"
                          max="18"
                          step="1"
                          value={entryEditorTextPseudoPt}
                          class="border-surface-700 bg-surface-900 text-surface-200 h-5 w-11 rounded border px-1 text-center text-[10px] font-semibold"
                          title={`${entryEditorTextBaseRem.toFixed(1)}rem`}
                          onblur={(e) => void setEntryEditorPseudoPt(Number(e.currentTarget.value))}
                          onkeydown={(e) => {
                            if (e.key === 'Enter') {
                              e.preventDefault()
                      ...
                      <button
                        ...
                        onclick={() => void nudgeEntryEditorTextBaseRem(0.1)}
                        disabled={entryEditorTextBaseRem >= 1.8}
              ...
              <div class="flex-1 overflow-y-auto p-6">
                <div class={cn('mx-auto w-full', entryEditorWidthClass)}>
                  <VaultLorebookEntryFields
                    data={selectedEntry}
                    descriptionTextBaseRem={entryEditorTextBaseRem}
