<script lang="ts">
  ...
  let contentRef = $state<{ discardUnsavedChanges: () => void } | null>(null)
  ...
  function handleEscapeKeydown(e: KeyboardEvent) {
    if (!contentHasChanges) return
    e.preventDefault()
    e.stopPropagation()
    suppressNextClose = true
    isOpen = true
    contentRef?.discardUnsavedChanges()
    ui.showToast('Unsaved edits discarded', 'info')
  }
</script>
