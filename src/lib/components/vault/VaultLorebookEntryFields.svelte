<script lang="ts">
  ...
  interface Props {
    ...
    descriptionTextBaseRem?: number
  }

  let { data, onUpdate, changedFields, descriptionTextBaseRem = 1.0 }: Props = $props()
...
<div class="space-y-6">
  ...
  <div
    class="flex flex-1 flex-col space-y-2 {changed('description')}"
    style={`--text-base: ${descriptionTextBaseRem.toFixed(1)}rem;`}
  >
