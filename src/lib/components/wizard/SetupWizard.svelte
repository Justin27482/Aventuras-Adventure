<script lang="ts">
  ...
  interface Props {
    onClose: () => void
    prefillFromStory?: Story | null
  }

  let { onClose, prefillFromStory = null }: Props = $props()
  ...
  $effect(() => {
    if (prefillApplied || !prefillFromStory) return
    prefillApplied = true
    wizard
  ...
  function blockOutsideClose(event: Event) {
    event.preventDefault()
  }
    ...
    <div class="min-h-0 flex-1 overflow-y-auto p-4">
      ...
      {:else if wizard.currentStep === 9}
        <Step8Opening
          ...
          onUseCardOpening={() =>
            wizard.narrative.useCardOpening({
              firstMessage: wizard.character.cardImportedFirstMessage,
              alternateGreetings: wizard.character.cardImportedAlternateGreetings,
              selectedGreetingIndex: wizard.character.selectedGreetingIndex,
              title: wizard.character.cardImportedTitle,
            })}
          onClearCardOpening={() => {
            wizard.clearScenarioLinkedLorebook()
            wizard.character.clearCardImport()
