...
<div class="space-y-5">
  <div>
    ...
    <p class="text-muted-foreground mt-1 text-sm">
      Select reusable global profiles for this campaign. Leave characters unassigned for human or GM
      control.
    </p>
  </div>
  ...
  {#if aiPlayers.length === 0}
    <Card>
      <CardContent
        class="text-muted-foreground flex min-h-32 flex-col items-center justify-center gap-2 text-center text-sm"
      >
        <Bot class="h-7 w-7" />
    ...
    <Card>
      <CardHeader>
        <CardTitle class="flex items-center gap-2 text-base"
          ><Bot class="h-4 w-4" /> Session Zero roster</CardTitle
        >
      </CardHeader>
  ...
  {:else}
    <Card>
      <CardHeader>
        <CardTitle class="flex items-center gap-2 text-base"
          ><Bot class="h-4 w-4" /> Character assignments</CardTitle
        >
      </CardHeader>
      <CardContent class="space-y-3">
        {#each characters as character (character.name)}
          <div class="grid gap-2 rounded-md border p-3 sm:grid-cols-[1fr_1fr] sm:items-center">
            <div>
              ...
              <p class="text-muted-foreground text-xs">
                {character.relationship === 'self' ? 'Primary character' : 'Party character'}
              </p>
            </div>
            <div class="space-y-1">
              <Label class="sr-only" for={`ai-player-${character.name}`}
                >AI Player for {character.name}</Label
              >
              <select
