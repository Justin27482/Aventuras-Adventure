<script lang="ts">
  ...
  function commitTone(nextTone: string) {
    const trimmed = nextTone.trim()
    const normalizedCurrent = tone.trim()
  ...
  <section class="space-y-2 pt-1">
    <div class="grid w-full items-center gap-2">
      <Input
        ...
        id="tone"
        value={toneDraft}
        oninput={(e) => (toneDraft = e.currentTarget.value)}
        onblur={() => commitTone(toneDraft)}
        onkeydown={(e) => {
          if (e.key === 'Enter') {
            e.preventDefault()
            commitTone(toneDraft)
          }
        }}
    ...
    <div class="flex flex-wrap gap-2">
      {#each ['Dark Fantasy', 'High Adventure', 'Cozy', 'Horror', 'Cyberpunk', 'Mystery'] as t (t)}
        <Button variant="outline" size="sm" class="h-7 text-xs" onclick={() => commitTone(t)}>
          {t}
        </Button>
      {/each}
    </div>
