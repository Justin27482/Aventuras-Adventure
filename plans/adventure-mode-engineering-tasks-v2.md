...
- [x] 3.7 Add encumbrance and slot/carry validation — implemented as slot-based carry limits (one equipped item per named slot per owner); full weight/encumbrance was out of scope since `Item` has no weight field and adding one is a broader migration
...
**Status: Core turn-order runtime and scene/actor abstraction are in place; scene/turn persistence foundation is implemented, while lifecycle wiring, turn-director logic, and UI affordances remain pending.**
...
- [~] 4.6 Add turn-order persistence and encounter order save/restore — additive database storage and TurnOrderService snapshot/restore are implemented; campaign lifecycle wiring remains
...
- [ ] 4.13 Add companion decision proposal flow with actor-owned rationale
- [ ] 4.14 Add autonomous companion combat policy
- [ ] 4.15 Add tactical delegation policy for optional player-issued companion intent
- [ ] 4.16 Add explicit tactical player-control policy for optional companion combat control
- [ ] 4.17 Prevent enemy and ordinary NPC actors from entering player-control paths
- [ ] 4.18 Add tests proving companion narrative agency survives tactical control settings
...
- [ ] 6.3 Introduce a new prompt-pack category for safety content: `safety-core-rules`, `safety-guardrails`, `safety-content-intensity`, `safety-content-bans`, `safety-mechanics-constraints`
- [ ] 6.4 Move all hardcoded safety instructions, consent boundaries, and dark-content policies into the new safety pack category
- [ ] 6.5 Add role-specific templates: GM core, world charter, ruleset digest, party roster, turn order
- [ ] 6.6 Add scene partials for overworld, dungeon, settlement, camp, combat, travel, and downtime
- [ ] 6.7 Add narrative turn templates for narration, NPC action, roll request, action resolution, QA, scene transition, montage
- [ ] 6.8 Add context variables for `sceneMode`, `turnType`, `activeActorName`, `turnOrderMode`, `upcomingActors`, `partyRoster`, `primaryCharacterName`, `companionRoster`, `companionAgency`, `controlPolicy`, `pendingRoll`, `recentRolls`
- [ ] 6.9 Add pack-template compatibility checks and repair flow for user-authored packs
- [ ] 6.10 Remove legacy protagonist references from default prompt context
- [ ] 6.11 Add GM party roster partial and other reusable shared partials
- [ ] 6.12 Add a prompt-pack test harness to compare safety variants without changing app code
...
- [ ] 10.3 Build `PartyPanel.svelte` showing ally pool, active party, primary character, agency, and spotlight indicators
...
- [ ] 10.7 Update `ActionInput.svelte` with primary-character context, companion requests, and scene indicator
- [ ] 10.8 Add “Primary character: X” UI and free-text blocking when roll is outstanding
...
- [ ] 10.11 Add campaign/session controls for companion combat policy
- [ ] 10.12 Add companion decision proposal and accept/modify/decline UI
